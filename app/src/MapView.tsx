...
import baseStyle from "./generated/style.json";
import { SOURCES } from "./sources";
import { TERRAIN_LAYERS } from "./terrain-layers";
...
// baseStyle (src/generated/style.json, produced by `npm run build:style`) is
// already a complete, portable MapLibre style -- real tile URLs, attribution,
// sprite, and glyphs are baked in at build time. The only thing added at
// runtime is dem/contours: maplibre-contour serves them through a custom,
// non-http protocol (registered below via demSource.setupMaplibre), which no
// other MapLibre consumer (Maputnik included) can resolve -- so they're kept
// out of the portable style file and injected here instead.
function getMaplibreStyle(demSource: any): StyleSpecification {
  ...
  style.layers.push(...TERRAIN_LAYERS);
...
function MapLibreView() {
  ...
  onMount(async () => {
    ...
    const africaBounds: LngLatBoundsLike = [
      [-19.1, -36.1],
      [52.6, 38.4],
    ];
    ...
    setTimeout(() => {
      ...
      if (attrControl) {
        attrControl.setAttribute(
          "title",
          "Click for full attribution and data sources",
        );
    ...
    map.on("load", () => {
      ...
      map.getContainer().addEventListener("change", (e: Event) => {
        ...
        if (
          cb.type === "checkbox" &&
          cb.checked &&
          cb.closest(".maplibregl-legend-list")
        ) {
